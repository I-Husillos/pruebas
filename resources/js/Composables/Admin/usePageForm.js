import { ref } from 'vue'
import { router } from '@inertiajs/vue3'
import { buildPagePayload } from '@/Composables/Admin/usePageLocalizations'
import { usePageCrud } from '@/Composables/Admin/usePageCrud'

export function usePageForm({ api, onSuccess = null }) {
    const errors     = ref({})
    const processing = ref(false)

    const { createPage, updatePage, deleteLocalization } = usePageCrud(api)

    async function submitCreate(formValue) {
        const payload = buildPagePayload(formValue)

        if (payload.localizations.length === 0) {
            errors.value = { general: 'Debes rellenar al menos una localización con slug.' }
            return false
        }

        return await _submit(() => createPage(payload))
    }

    async function submitUpdate(pageId, formValue) {
        const payload = buildPagePayload(formValue)
        return await _submit(() => updatePage(pageId, payload))
    }

    async function removeLocalization(localizationId) {
        try {
            await deleteLocalization(localizationId)
            return { ok: true }
        } catch (e) {
            return {
                ok:      false,
                message: e.response?.data?.message || 'Error al eliminar la localización.',
            }
        }
    }

    async function _submit(apiCall) {
        processing.value = true
        errors.value     = {}
        try {
            await apiCall()
            if (onSuccess) {
                onSuccess()
            } else {
                router.visit(route('admin.pages.index'))
            }
            return true
        } catch (e) {
            errors.value = e.response?.status === 422
                ? (e.response.data.errors || {})
                : { general: 'Error inesperado al guardar.' }
            return false
        } finally {
            processing.value = false
        }
    }

    return { errors, processing, submitCreate, submitUpdate, removeLocalization }
}