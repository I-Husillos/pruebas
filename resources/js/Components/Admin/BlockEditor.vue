<template>
    <div class="space-y-6">
        ...
        <div v-for="(row, rIndex) in rows" :key="rIndex"
            class="border-2 border-dashed border-gray-300 rounded-lg p-4 relative hover:border-indigo-300 transition-colors bg-gray-50/50">
            <!-- Row Controls -->
            <div class="absolute right-2 top-2 flex gap-2 opacity-50 hover:opacity-100 transition-opacity z-10">
                <select v-model="row.layout" class="text-xs border-gray-300 rounded shadow-sm py-1 pl-2 pr-6"
                ...
                <button type="button" @click="moveRow(rIndex, -1)" v-if="rIndex > 0"
                    class="bg-white p-1 rounded border shadow-sm text-gray-500 hover:text-indigo-600">↑</button>
                <button type="button" @click="moveRow(rIndex, 1)" v-if="rIndex < rows.length - 1"
                    class="bg-white p-1 rounded border shadow-sm text-gray-500 hover:text-indigo-600">↓</button>
                <button type="button" @click="removeRow(rIndex)"
                    class="bg-white p-1 rounded border shadow-sm text-red-400 hover:text-red-600">×</button>
            </div>

            <!-- Columns -->
            <div class="grid gap-4 mt-8" :class="getGridClass(row.layout)">
                <div v-for="(col, cIndex) in row.columns" :key="cIndex"
                    class="bg-white border rounded-md p-3 min-h-[100px] flex flex-col gap-3">
                    <span class="text-xs text-gray-300 font-mono uppercase text-center block mb-2">Columna {{ cIndex + 1
                        }}</span>

                    <!-- Blocks in Column -->
                    <div v-for="(block, bIndex) in col.blocks" :key="bIndex"
                        class="relative group border rounded p-2 hover:shadow-sm">
                        <!-- Block Controls -->
                        <div
                            class="absolute right-1 top-1 flex gap-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button type="button" @click="moveBlock(col, bIndex, -1)" v-if="bIndex > 0"
                                class="bg-white text-gray-400 hover:text-indigo-500 text-xs px-1 border rounded">↑</button>
                            <button type="button" @click="moveBlock(col, bIndex, 1)"
                                v-if="bIndex < col.blocks.length - 1"
                                class="bg-white text-gray-400 hover:text-indigo-500 text-xs px-1 border rounded">↓</button>
                            <button type="button" @click="removeBlock(col, bIndex)"
                                class="bg-white text-red-400 hover:text-red-600 text-xs px-1 border rounded">×</button>
                        </div>

                        <!-- Block Content -->
                        <div class="pt-4">
                            <span class="text-[10px] font-bold uppercase text-indigo-400 mb-1 block">{{
                                getBlockLabel(block.type) }}</span>

                            <WysiwygBlock v-if="['rich_text', 'wysiwyg'].includes(block.type)" v-model="block.data" />
                            <MultimediaBlock v-if="block.type === 'multimedia'" v-model="block.data" :market="market" />
                            <FormBlock v-if="block.type === 'form'" v-model="block.data" :forms="forms" />

                            <!-- Legacy/Other types -->
                            <div v-if="block.type === 'html'">
                                <textarea v-model="block.data.code" rows="4"
                                    class="w-full bg-gray-900 text-white text-xs font-mono rounded p-2"></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-auto pt-2 border-t border-dashed border-gray-200">
                        <div class="flex flex-wrap gap-1 justify-center">
                            <button type="button" @click="addBlock(col, 'rich_text')"
                                class="text-xs px-2 py-1 bg-gray-50 hover:bg-gray-100 border rounded flex items-center gap-1 text-gray-600">
                                <span>T</span> Texto
                            </button>
                            <button type="button" @click="addBlock(col, 'multimedia')"
                                class="text-xs px-2 py-1 bg-gray-50 hover:bg-gray-100 border rounded flex items-center gap-1 text-gray-600">
                                <span>📷</span> Multimedia
                            </button>
                            <button type="button" @click="addBlock(col, 'form')"
                                class="text-xs px-2 py-1 bg-gray-50 hover:bg-gray-100 border rounded flex items-center gap-1 text-gray-600">
                                <span>📝</span> Form
                            </button>
                            <button type="button" @click="addBlock(col, 'html')"
                                class="text-xs px-2 py-1 bg-gray-50 hover:bg-gray-100 border rounded flex items-center gap-1 text-gray-600">
                                <span>&lt;/&gt;</span> HTML
                            </button>
                        </div>
        ...
        <!-- Add Row Button -->
        <div class="text-center py-4 border-2 border-dashed border-gray-200 rounded-lg hover:border-indigo-300 hover:bg-indigo-50 transition-all cursor-pointer"
            @click="addRow">
