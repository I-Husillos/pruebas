<template>
  <AdminLayout>
    ...
    <div class="max-w-4xl">
      <h1 class="text-3xl font-bold text-gray-900 mb-8">Editar Widget</h1>
      ...
      <form @submit.prevent="submit" class="space-y-8">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div>
            ...
            <select v-model="form.zone_id" :class="{'border-red-300 focus:border-red-500 focus:ring-red-500': form.errors.zone_id}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <p v-if="form.errors.type" class="mb-2 text-sm text-red-600">{{ form.errors.type }}</p>
        ...
        <div v-if="form.type" class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div v-if="form.type === 'menu'" class="space-y-4">
            <div>
              ...
              <select v-model="form.config.menu_id" :class="{'border-red-300 focus:border-red-500 focus:ring-red-500': form.errors['config.menu_id']}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                <option :value="null">Seleccionar menú...</option>
                <option v-for="menu in menus" :key="menu.id" :value="menu.id">
                  {{ menu.name?.es || menu.name?.en }}
                </option>
              </select>
              <p v-if="form.errors['config.menu_id']" class="mt-1 text-sm text-red-600">{{ form.errors['config.menu_id'] }}</p>
            </div>
            <div>
          ...
          <div v-if="form.type === 'form'" class="space-y-4">
            <div>
              ...
              <select v-model="form.config.form_id" :class="{'border-red-300 focus:border-red-500 focus:ring-red-500': form.errors['config.form_id']}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                <option :value="null">Seleccionar formulario...</option>
                <option v-for="formItem in forms" :key="formItem.id" :value="formItem.id">
                  {{ formItem.name }}
                </option>
              </select>
              <p v-if="form.errors['config.form_id']" class="mt-1 text-sm text-red-600">{{ form.errors['config.form_id'] }}</p>
            </div>
          </div>
          ...
          <div v-if="form.type === 'fixed_content'" class="space-y-4">
            <div>
              ...
              <input v-model="form.config.content_key" type="text" :class="{'border-red-300 focus:border-red-500 focus:ring-red-500': form.errors['config.content_key']}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="footer-info" required />
              <p v-if="form.errors['config.content_key']" class="mt-1 text-sm text-red-600">{{ form.errors['config.content_key'] }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input v-model="form.title.es" type="text" :class="{'border-red-300 focus:border-red-500 focus:ring-red-500': form.errors['title.es']}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
...
onMounted(() => {
  form.zone_id = props.widget.zone_id;
  form.type = props.widget.type;
  form.title = {
    es: props.widget.title?.es || '',
    en: props.widget.title?.en || ''
  };
  form.config = props.widget.config || {};
  form.sort_order = props.widget.sort_order || 0;
  form.active = props.widget.active !== 0; // Ensure boolean
});

// Reset config when type changes (optional, but might want to preserve if switching back and forth is needed, but typically good for clean state)
// Actually for Edit, we might want to be careful not to wipe it if they just clicked it by accident. 
// But if they actually change the type, the config schema changes.
...
const submit = () => {
  form.put(route('admin.widgets.update', props.widget.id));
};
</script>
