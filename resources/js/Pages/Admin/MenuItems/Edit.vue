<template>
  <AdminLayout>
    ...
    <div class="max-w-4xl">
      <h1 class="text-3xl font-bold text-gray-900 mb-8">Editar Item de Menú</h1>
          ...
          <div class="space-y-4">
            <div>
              ...
              <select v-model="form.parent_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                ...
                <option v-for="pItem in parentItems" :key="pItem.id" :value="pItem.id">
                  {{ pItem.label?.es || pItem.label?.en }}
                </option>
              </select>
            </div>
            <div>
...
onMounted(() => {
  form.label.es = props.item.label?.es || '';
  form.label.en = props.item.label?.en || '';
  form.url = props.item.url || '';
  form.route_name = props.item.route_name || '';
  form.route_params = props.item.route_params || {};
  form.parent_id = props.item.parent_id;
  form.icon = props.item.icon || '';
  form.target = props.item.target || '_self';
  form.sort_order = props.item.sort_order || 0;
  form.active = props.item.active !== 0; // Ensure boolean
});

const submit = () => {
  form.put(route('admin.menu-items.update', props.item.id));
};
</script>
