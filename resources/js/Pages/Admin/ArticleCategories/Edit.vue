...
const normalizeLocalizedFields = (category) => {
  ...
  if (Array.isArray(category.translations)) {
    const es = category.translations.find((t) => Number(t.language_id) === 1) || {};
    const en = category.translations.find((t) => Number(t.language_id) === 2) || {};
...
const submit = async () => {
  ...
  try {
    await api.put(`/api/v1/article-categories/${props.category.id}`, payload);
